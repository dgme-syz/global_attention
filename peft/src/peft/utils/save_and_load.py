...
def get_peft_model_state_dict(
    ...
    if state_dict is None:
        state_dict = model.state_dict()
    if config.peft_type is PeftType.GA:
        to_return = {k: state_dict[k] for k in state_dict if "ga_" in k}
        to_return = {k: v for k, v in to_return.items() if (("ga_" in k and adapter_name in k))}
